...
const ReviewerPage = ({ onSubmit, onEdit, onDelete }) => {
  // Mock existing reviewers data
  const [reviewers, setReviewers] = useState([
    {
      id: 1,
      reviewer_name: "Binod bhavan",
      reviewer_email: "binod@gmail.com",
      reviewer_designation: "developer",
      is_active: true,
    ...
    },
    {
      ...
      reviewer_designation: "Senior Reviewer",
      is_active: true,
      reviewer_password: "securepass456",
    ...
    },
    {
      ...
      reviewer_designation: "Technical Reviewer",
      is_active: false,
      reviewer_password: "mypassword789",
    ...
    if (editingReviewer) {
      setReviewers(prev => prev.map(reviewer => 
        reviewer.id === editingReviewer.id 
          ? { ...reviewer, ...values, updated_at: new Date().toISOString().split('T')[0] }
          : reviewer
      ));
  ...
  const handleDeleteReviewer = (reviewer) => {
    ...
    if (onDelete) onDelete(reviewer);
  };

  const toggleReviewerStatus = (reviewerId) => {
    setReviewers(prev => prev.map(reviewer => 
      reviewer.id === reviewerId 
        ? { ...reviewer, is_active: !reviewer.is_active }
        : reviewer
    ));
  };
    ...
    return (
      <div className="max-w-4xl mx-auto p-6 bg-white">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <Formik
            initialValues={editingReviewer ? {
              reviewer_name: editingReviewer.reviewer_name || '',
              reviewer_email: editingReviewer.reviewer_email || '',
              reviewer_designation: editingReviewer.reviewer_designation || '',
              is_active: editingReviewer.is_active !== undefined ? editingReviewer.is_active : true,
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-gray-500">
                      ({values.is_active ? 'Reviewer can access review assignments' : 'Reviewer access is disabled'})
                    </span>
                  </div>
                  ...
                  <div className="flex gap-3 pt-4 border-t border-gray-200">
                    ...
                    >
                      ...
                      {isSubmitting ? 'Saving...' : editingReviewer ? 'Update Reviewer' : 'Save Reviewer'}
                    </button>
                    <button
                      type="button"
                      onClick={() => setShowForm(false)}
    ...
    <div className="max-w-7xl mx-auto p-6 bg-white">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-bold text-gray-900">Reviewers Management</h1>
          </div>
          <p className="text-gray-600">Manage reviewers and their review permissions</p>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
        {reviewers.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-400">Add your first reviewer to get started</p>
          </div>
        ) : (
          <>
            {/* Table Header */}
                  ...
                  <div className="grid grid-cols-12 gap-4 items-center">
                    ...
                    <div className="col-span-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-900 text-sm">{reviewer.reviewer_email}</span>
                      </div>
                    </div>
                    ...
                    <div className="col-span-1">
                      <div className="flex items-center space-x-1">
                        <button
                          onClick={() => toggleReviewerStatus(reviewer.id)}
                          className={`p-1.5 rounded-lg transition-colors ${
                            reviewer.is_active 
                              ? 'text-green-600 hover:bg-green-50' 
                              : 'text-red-600 hover:bg-red-50'
                          }`}
                          title={reviewer.is_active ? 'Deactivate' : 'Activate'}
                        >
      ...
      <div className="mt-6 bg-gray-50 rounded-lg p-4">
        <div className="flex items-center justify-between text-sm text-gray-600">
          <span>Total Reviewers: {reviewers.length}</span>
          <span>Active Reviewers: {reviewers.filter(r => r.is_active).length}</span>
